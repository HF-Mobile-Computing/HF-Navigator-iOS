...
@interface AKNavColor4 : UIColor
+(UIColor *)backgroundColor;
+(UIColor *)lineColor;
+(UIColor *)selectionColor;
@end
@implementation AKNavColor4

+(UIColor *)backgroundColor{
    return [UIColor colorWithHexString:@"#4AB86E"];
}
+(UIColor *)lineColor{
...
@interface AKNavButton4 : UIButton
@property (nonatomic,assign) NSInteger index;
@end
@implementation AKNavButton4

@end
#pragma mark - AKNavTabView
@protocol AKNavTabViewDelegate4 <NSObject>
@required
-(void) navButtonClicked:(AKNavButton4 *)navButton;

@end

@interface AKNavTabView4 : UIView
@property(nonatomic,strong) NSArray *titles;
@property(nonatomic,weak) id <AKNavTabViewDelegate4> delegate;
@end
@implementation AKNavTabView4{
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        self.backgroundColor=[AKNavColor4 backgroundColor];
...
-(void)drawRect:(CGRect)rect{
    ...
    CGContextSetFillColorWithColor(context, [AKNavColor4 lineColor].CGColor);
...
-(void) setTitles:(NSArray *)titles{
    ...
    for (UIView *subview in _scrollView.subviews) {
        if ([subview isKindOfClass:[AKNavButton4 class]]) {
    ...
    if (!_selectionLine) {
        ...
        [_selectionLine setBackgroundColor:[AKNavColor4 selectionColor]];
...
//    NSInteger titlesCount = _titles.count;
    ...
    CGFloat _buttonWidth = _width / 3.5;
    ...
    for (NSString *title in _titles) {
        ...
        if (_index == 0) {
            AKNavButton4 *button = [AKNavButton4 buttonWithType:UIButtonTypeCustom];
            button.index=_index;
            ...
            CGRect frame = button.frame;
            frame.size.height=buttonHeight;
            frame.size.width=_buttonWidth / 1.2;
            button.frame=frame;
            [button setTitle:title forState:UIControlStateNormal];
            [button addTarget:self action:@selector(navButtonAction:) forControlEvents:UIControlEventTouchDown];
            [button setShowsTouchWhenHighlighted:NO];
...
-(void) navButtonAction:(AKNavButton4 *)navButton{
...
@end
#pragma mark - HFController
@interface HFController ()<AKNavTabViewDelegate4>

@end

@implementation HFController {
    AKNavTabView4 *_tabView;
...
- (void)viewDidLoad {
    ...
    //#Core properties ```
    CGFloat _width = CGRectGetWidth([ UIScreen mainScreen].bounds);
    _tabView=[[AKNavTabView4 alloc] initWithFrame:CGRectMake(0, 0, _width, tabHeight)];
    _tabView.delegate=self;
    [self.view addSubview:_tabView];
    [self setTitles:@[@"Calendar", @"Library", @"About"]];
    [self setViewControllersId:@[@"calController", @"libController", @"aboutController"]];
    
    self.navigationController.navigationBar.barTintColor = [UIColor colorWithHexString:@"#4AB86E"];
