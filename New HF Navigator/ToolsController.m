...
@interface AKNavColor3 : UIColor
+(UIColor *)backgroundColor;
+(UIColor *)lineColor;
+(UIColor *)selectionColor;
@end
@implementation AKNavColor3

+(UIColor *)backgroundColor{
    return [UIColor colorWithHexString:@"#E64545"];
}
+(UIColor *)lineColor{
...
@interface AKNavButton3 : UIButton
@property (nonatomic,assign) NSInteger index;
@end
@implementation AKNavButton3

@end
#pragma mark - AKNavTabView
@protocol AKNavTabViewDelegate3 <NSObject>
@required
-(void) navButtonClicked:(AKNavButton3 *)navButton;

@end

@interface AKNavTabView3 : UIView
@property(nonatomic,strong) NSArray *titles;
@property(nonatomic,weak) id <AKNavTabViewDelegate3> delegate;
@end
@implementation AKNavTabView3{
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        self.backgroundColor=[AKNavColor3 backgroundColor];
...
-(void)drawRect:(CGRect)rect{
    ...
    CGContextSetFillColorWithColor(context, [AKNavColor3 lineColor].CGColor);
...
-(void) setTitles:(NSArray *)titles{
    ...
    for (UIView *subview in _scrollView.subviews) {
        if ([subview isKindOfClass:[AKNavButton3 class]]) {
    ...
    if (!_selectionLine) {
        ...
        [_selectionLine setBackgroundColor:[AKNavColor3 selectionColor]];
    ...
    CGFloat _buttonWidth = _width/2.5;
    ...
    for (NSString *title in _titles) {
        ...
        if (_index == 0) {
            AKNavButton3 *button = [AKNavButton3 buttonWithType:UIButtonTypeCustom];
            button.index=_index;
            ...
            CGRect frame = button.frame;
            frame.size.height=buttonHeight;
            frame.size.width=_buttonWidth / 1.5;
            button.frame=frame;
            [button setTitle:title forState:UIControlStateNormal];
            [button addTarget:self action:@selector(navButtonAction:) forControlEvents:UIControlEventTouchDown];
            [button setShowsTouchWhenHighlighted:NO];
            ...
            CGRect frame = button.frame;
            frame.size.height=buttonHeight;
            frame.size.width=_buttonWidth / 1.6;
            button.frame=frame;
            [button setTitle:title forState:UIControlStateNormal];
            [button addTarget:self action:@selector(navButtonAction:) forControlEvents:UIControlEventTouchDown];
            [button setShowsTouchWhenHighlighted:NO];
...
-(void) navButtonAction:(AKNavButton3 *)navButton{
...
@end
#pragma mark - ToolsController
@interface ToolsController ()<AKNavTabViewDelegate3>

@end

@implementation ToolsController {
    AKNavTabView3 *_tabView;
...
- (void)viewDidLoad {
    ...
    //#Core properties ```
    CGFloat _width = CGRectGetWidth([ UIScreen mainScreen].bounds);
    _tabView=[[AKNavTabView3 alloc] initWithFrame:CGRectMake(0, 0, _width, tabHeight)];
    _tabView.delegate=self;
    [self.view addSubview:_tabView];
    [self setTitles:@[@"Castle Learning", @"Portal", @"Edmodo"]];
    [self setViewControllersId:@[@"castleController", @"portalController", @"edmodoController"]];
    
    self.navigationController.navigationBar.barTintColor = [UIColor colorWithHexString:@"#E64545"];
