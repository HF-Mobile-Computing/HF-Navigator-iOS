...
@interface AKNavColor2 : UIColor
+(UIColor *)backgroundColor;
+(UIColor *)lineColor;
+(UIColor *)selectionColor;
@end
@implementation AKNavColor2

+(UIColor *)backgroundColor{
    return [UIColor colorWithHexString:@"#3366FF"];
}
+(UIColor *)lineColor{
...
@interface AKNavButton2 : UIButton
@property (nonatomic,assign) NSInteger index;
@end
@implementation AKNavButton2

@end
#pragma mark - AKNavTabView
@protocol AKNavTabViewDelegate2 <NSObject>
@required
-(void) navButtonClicked:(AKNavButton2 *)navButton;

@end

@interface AKNavTabView2 : UIView
@property(nonatomic,strong) NSArray *titles;
@property(nonatomic,weak) id <AKNavTabViewDelegate2> delegate;
@end
@implementation AKNavTabView2{
...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        self.backgroundColor=[AKNavColor2 backgroundColor];
...
-(void)drawRect:(CGRect)rect{
    ...
    CGContextSetFillColorWithColor(context, [AKNavColor2 lineColor].CGColor);
...
-(void) setTitles:(NSArray *)titles{
    ...
    for (UIView *subview in _scrollView.subviews) {
        if ([subview isKindOfClass:[AKNavButton2 class]]) {
    ...
    if (!_selectionLine) {
        ...
        [_selectionLine setBackgroundColor:[AKNavColor2 selectionColor]];
    ...
    CGFloat _buttonWidth = _width/5.0;
    
    CGFloat _pointer = 20.0;
    if (titlesCount == 1) {
        _buttonWidth = _width;
    }else if (titlesCount==2){
        _buttonWidth = _width/2.5;
    }
    ...
    for (NSString *title in _titles) {
        ...
        if (_index == 0) {
            AKNavButton2 *button = [AKNavButton2 buttonWithType:UIButtonTypeCustom];
            button.index=_index;
            ...
            CGRect frame = button.frame;
            frame.size.height=buttonHeight;
            frame.size.width=_buttonWidth + 20;
            button.frame=frame;
            [button setTitle:title forState:UIControlStateNormal];
            [button addTarget:self action:@selector(navButtonAction:) forControlEvents:UIControlEventTouchDown];
            [button setShowsTouchWhenHighlighted:NO];
...
-(void) navButtonAction:(AKNavButton2 *)navButton{
...
@end
#pragma mark - ResController
@interface ResController ()<AKNavTabViewDelegate2>

@end

@implementation ResController {
    AKNavTabView2 *_tabView;
...
- (void)viewDidLoad {
    ...
    //#Core properties ```
    CGFloat _width = CGRectGetWidth([ UIScreen mainScreen].bounds);
    _tabView=[[AKNavTabView2 alloc] initWithFrame:CGRectMake(0, 0, _width, tabHeight)];
    _tabView.delegate=self;
    [self.view addSubview:_tabView];
    [self setTitles:@[@"Bell Schedule", @"Exam Schedules"]];
    [self setViewControllersId:@[@"bellController", @"examController"]];
    
    self.navigationController.navigationBar.barTintColor = [UIColor colorWithHexString:@"#3366FF"];
